...
public:
    ...
    static void spawn(Callable && f, Args &&... args) {
...
public:
    ...
    explicit xthreadsafe_queue(std::size_t const max_size = std::numeric_limits<std::size_t>::max()) : m_max_size{max_size} {}

    void push(T && o) {
        LOCK_GUARD(m_queue_mutex) {
